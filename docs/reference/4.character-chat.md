...
actor 사용자
participant "캐릭터 채팅 페이지" as Page
participant "Cinemate 서버" as Server
database "Supabase DB" as DB
participant "OpenAI API" as LLM

사용자 -> Page: 영화와 캐릭터 선택
Page -> Server: 대화 준비 요청
Server -> DB: 캐릭터 정보와 대화 상태 조회
DB --> Server: 인사말·대화 기록·추천 질문
Server -> DB: 필요한 경우 대화 세션 생성
Server --> Page: 대화 상태 반환
Page --> 사용자: 인사말·대화 기록·추천 질문 표시

사용자 -> Page: 메시지 입력
Page -> Server: 메시지 전송
Server -> DB: persona 프롬프트·사건별 관점·최근 8개 메시지 조회
DB --> Server: 캐릭터 응답 context
Server -> LLM: context와 현재 질문 전달
LLM --> Server: 캐릭터 응답과 후속 질문
Server -> DB: 사용자 메시지와 캐릭터 응답 저장
Server --> Page: 응답과 후속 질문
Page --> 사용자: 캐릭터 답변 표시
