...
- 캐릭터 응답의 후속 질문 후보를 저장해 대화 복원 시 다시 표시한다.
...
group 기존 대화 복원
  User -> FE: 캐릭터 재선택
  FE -> Route: GET /conversations?movieId&characterId
  Route -> Service: getConversation(context, input)
  Service -> Repo: 최신 conversation과 메시지 조회
  Repo -> DB: conversation, messages 조회
  DB --> Repo: rows
  Repo --> Service: conversation state
  Service --> Route: initialMessage, messages, suggestedQuestions
  Route --> FE: conversation state
  FE --> User: 기존 대화와 후속 질문 표시
end
...
group 메시지 전송
  ...
  Service -> Service: 캐릭터 관련 사건 context 구성
...
- `GET /api/character-chat/conversations?movieId=&characterId=`는 선택한 영화/캐릭터 조합에 대한 로그인 사용자의 최신 conversation을 조회한다.
- 기존 conversation이 없으면 `conversationId: null`, 빈 `messages`, `characters.greeting` 기반 `initialMessage`, 캐릭터별 기본 질문을 반환한다.
- 기존 conversation이 있으면 저장된 메시지를 생성 시각 오름차순으로 반환한다.
- `suggestedQuestions`는 마지막 캐릭터 메시지에 저장된 후속 질문을 반환한다. 저장값이 없으면 기본 질문으로 fallback한다.
...
- 최근 대화, 캐릭터 정보, 영화 정보, 현재 캐릭터의 관련 사건/관점 데이터를 조회한다.
- seed된 persona와 현재 캐릭터의 관련 사건/관점 전체를 조립해 LLM client가 캐릭터 응답과 후속 질문 후보를 생성한다.
- LLM 호출이 실패하면 잘못된 캐릭터 응답 메시지를 저장하지 않는다.
- 사용자 메시지를 먼저 저장할지, LLM 성공 후 함께 저장할지는 구현 시 트랜잭션 처리와 재시도 정책을 기준으로 결정한다.
- 캐릭터 응답 메시지에는 LLM이 생성한 후속 질문 후보를 정규화해 함께 저장한다.

## 관련 사건 Context 구성

관련 사건 context는 현재 캐릭터의 participant row를 기준으로 구성한다.

1. 현재 `character_id`의 `character_chat_event_participants`를 조회한다.
2. 연결된 `character_chat_events`를 사건 순서대로 조회한다.
3. 조회된 사건과 캐릭터 관점 데이터 전체를 LLM 입력에 포함한다.

현재 seed 규모에서는 키워드 매칭으로 사건을 제한하지 않는다. 사건 수 증가로 토큰 비용이나 맥락 품질 문제가 발생하면 별도의 검색 또는 요약 전략을 검토한다.
...
- 동적 `suggestedQuestions`는 캐릭터 응답 메시지의 `suggested_questions`에 저장한다.
