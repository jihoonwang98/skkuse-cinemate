...
| `movie_similarities` | 기준 영화별 Item CF 후보와 유사도 점수 |
...
`movie_similarities`는 seed 단계에서 생성된 결과만 조회한다. 런타임에서 유사도를 새로 계산하지 않는다.

## 주요 흐름

![시퀀스 다이어그램](assets/6.item-cf-implementation-plan.png)
...
seed 영화별로 `movie_similarities.source_movie_id`를 조회한다.
...
|---|---|
| `source` | `item_cf` |
| `score` | `movie_similarities.score` |
| `coRatingCount` | `movie_similarities.co_rating_count` |
...
| Item CF 정렬 | 후보는 `score DESC`, `co_rating_count DESC`, `target_movie_id ASC` 순서로 사용한다. |
...
| fallback 정렬 | fallback 후보는 `movielens_rating_count DESC`, `movielens_avg_rating DESC`, `movie_id ASC` 순서로 사용한다. |
