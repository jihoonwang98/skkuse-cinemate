...
![시퀀스 다이어그램](assets/2.movie-basic-data-exploration-implementation-plan.png)

![시퀀스 다이어그램](assets/2.movie-basic-data-exploration-implementation-plan-2.png)
...
- `limit`은 기본 50, 최대 50으로 제한한다.
- `/search`도 페이지네이션 없이 `q`와 `limit` 기준으로 최대 50개를 반환한다.
- 온보딩 후보는 화면 요구에 따라 `sort=popular&limit=...`로 조회한다.

정렬 기준:

| `sort` | 정렬 |
|---|---|
| `popular` | `movie_stats.movielens_rating_count DESC`, `movies.id ASC` |
| `rating` | `movie_stats.movielens_rating_count >= 10000` 조건 적용 후 계산 평점 DESC, `movie_stats.movielens_rating_count DESC`, `movies.id ASC` |
...
| `posterUrl` | `movies.poster_path`를 TMDB 이미지 URL로 변환, 없으면 `null` |
| `isBookmarked` | 비로그인 `false`, 로그인 사용자는 `movie_bookmarks` 조회 |

`poster_path`가 없으면 `posterUrl`은 `null`로 응답하고 프론트에서 fallback UI를 표시한다. `backdropUrl`, `trailerUrl`, 출연진 `profileUrl`도 원본 데이터가 없으면 `null`을 유지한다.
...
| `cast` | `movie_casts.cast_order ASC` 기준 전체 출연진 |
...
출연진은 `{ id, name, characterName, profileUrl }[]` 형태로 전체를 응답한다. 화면에서는 현재 UI에 맞춰 이름 중심으로 표시한다.
...
유사 영화 API는 이번 범위에서 구현하지 않는다. 상세 화면의 비슷한 영화 영역은 임시 하드코딩 값을 유지하고, Item-CF 기반 API 전환 TODO를 코드에 남긴다.
...
| 파일 | 역할 |
|---|---|
| `server/movies/movie-service.ts` | 목록, 상세, 장르 유스케이스 조립 |
...
목록 API에서는 N+1 조회를 피하기 위해 limit 결과의 movie id 목록을 만든 뒤, 해당 목록에 대한 사용자의 찜 row를 한 번에 조회한다.
...
| `/search` | 클라이언트 필터링을 `GET /api/movies?q&limit=50` 호출로 교체 |
| `/movie/[id]` | 하드코딩 상세 데이터를 `GET /api/movies/{movieId}` 결과로 교체 |
| `/onboarding` | 인기 영화 후보를 `GET /api/movies?sort=popular&limit=...` 결과로 교체 |

App Router 기준으로 페이지는 가능한 Server Component에서 초기 데이터를 조회한다. `/search`는 `q`를 URL query 기준으로 유지하고 페이지네이션은 제공하지 않는다.
...
| 대상 | 검증 |
|---|---|
| `movie-rules.test.ts` | 평점 계산, 이미지 URL 조립, limit 기본값, 검색어 trim |
| `movie-service.test.ts` | 비로그인 찜 false, 로그인 찜 병합, 목록 응답 조립, 상세 404 처리 |
| `movie-schema.test.ts` | query/path 검증 실패 케이스 |
| `/search` 표시 모델 테스트 | API 영화 카드 응답을 기존 카드 표시 props로 변환 |
...
5. 홈, 검색, 상세, 온보딩 화면의 하드코딩 데이터를 API 기반으로 교체한다.
6. rules/service/schema 테스트를 추가하고 `pnpm lint`를 실행한다.
