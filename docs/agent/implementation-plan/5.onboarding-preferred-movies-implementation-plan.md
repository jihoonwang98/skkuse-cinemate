...
`user_onboarding_movies.movie_id`와 `movies.id`는 모두 TMDB movie id를 기준으로 연결한다. 현재 스키마에서 `movies.movielens_id`는 `not null`이므로 별도 MovieLens 필터는 두지 않는다.

## 주요 흐름

![시퀀스 다이어그램](assets/5.onboarding-preferred-movies-implementation-plan.png)

## 구현 범위

### 온보딩 후보 영화 조회

`GET /api/movies?sort=popular&page=&size=`는 `/onboarding`에서 선택 가능한 영화 후보를 반환한다.

| 항목 | 기준 |
|---|---|
| 인증 | 필요 |
| Query | `sort=popular`, `page`, `size` |
| Response | `movies: MovieCard[]`, `page`, `size`, `totalCount` |

동작 기준:

- `/onboarding` 화면에서는 로그인 사용자인 경우에만 후보를 조회한다.
- 현재 스키마에서 `movies.movielens_id`는 `not null`이므로 별도 MovieLens 필터를 두지 않는다.
- 기본 정렬은 인기순으로 둔다.
- `size`는 화면에서 충분한 선택지를 제공하되 과도한 조회를 막기 위해 최대값을 둔다. 초기 최대값은 60으로 한다.
...
- 저장 전 현재 사용자의 기존 `user_onboarding_movies` row를 삭제한다. 같은 요청 반복 시 최종 상태는 동일하며, 다른 영화 또는 다른 순서 요청은 전체 교체로 반영한다.
...
- 미완료 사용자는 `GET /api/movies?sort=popular&page=&size=`로 후보 영화를 조회한다.
