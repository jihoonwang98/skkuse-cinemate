...
- Google 소셜 로그인을 Supabase Auth로 처리한다.
...
![시퀀스 다이어그램](assets/1.auth-session-implementation-plan.png)

## 구현 범위

### 로그인

- `/login` 화면의 Google 버튼은 Supabase client의 `signInWithOAuth`를 호출한다.
...
| `email` | Auth user email. 이메일이 없는 auth user는 profile을 생성하지 않고 실패 처리한다. |
...
6. `/login` 화면의 Google 로그인 버튼과 오류 상태를 구현한다.
...
| 항목 | 기준 |
|---|---|
| 로그인 시작 | Google 버튼이 Supabase OAuth 흐름을 시작한다. |
