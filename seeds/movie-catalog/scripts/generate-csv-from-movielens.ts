...
const SEED_DIR = path.join(ROOT_DIR, "seeds", "movie-catalog");
...
async function main() {
  ...
  if (selectedMovies.length < TARGET_MOVIE_COUNT) {
    ...
    throw new Error(
      `Only ${selectedMovies.length} movies satisfy the hard conditions. See seeds/movie-catalog/reports/selection-report.json`,
    );
  }
